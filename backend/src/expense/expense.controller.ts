...
export class ExpenseController {
  ...
  create(@Body() expenseData: CreateExpenseDto, @UserId() userId: string) {
    if (!userId) throw new UnauthorizedException('User ID required');
    return this.expenseService.create({ ...expenseData, date: new Date(expenseData.date), user: userId as any });
  }

  @Put(':id')
  update(@Param('id') id: string, @Body() expenseData: UpdateExpenseDto, @UserId() userId: string) {
    if (!userId) throw new UnauthorizedException('User ID required');
    return this.expenseService.update(id, { ...expenseData, date: new Date(expenseData.date) }, userId);
  }
